...
class AttentionPreClassify(nn.Module):
    def __init__(self,hidden_size):
        super(AttentionPreClassify, self).__init__()
        hidden_size = 128
    ...
    def forward(self, encoder_outputs, teacher_encoder_outputs):
        ...
        # print(student_feature.size(),encoder_outputs.size())
        x ,_ = self.multihead_attn(encoder_outputs,encoder_outputs,encoder_outputs)
        encoder_outputs = torch.mean(x.transpose(1,0),dim = 1)
        # print(encoder_outputs.size(),encoder_outputss.size())
        ...
        merged_outputs = torch.cat([encoder_outputs,teacher_encoder_outputs],dim = -1)
        # print(merged_outputs.size(),merged_outputss.size())
...
class PreClassify(nn.Module):
    def __init__(self, hidden_size):
        ...
        hidden_size = 128
        self.gru_input = nn.GRU(786, hidden_size, 2, dropout=0.4, bidirectional=True)
        self.hidden_size = hidden_size
...
class Bert_EncoderSeq(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, n_layers=2, dropout=0.5):
        ...
        self.hidden_size = hidden_size
        self.bert_rnn = BertModel.from_pretrained("/home/yjzhang/Paper/Model/Op10")
