...
train_data = json.load(open('data/MathQA_train.json', 'r', encoding='utf-8'))
test_data = json.load(open('data/MathQA_test.json', 'r', encoding='utf-8'))

train_pairs, generate_nums, copy_nums = transfer_mathqa_num(train_data)
test_pairs, _, _ = transfer_mathqa_num(test_data)
...
train_fold = temp_pairs

temp_pairs = []
for p in test_pairs:
    temp_pairs.append((p[0], from_infix_to_prefix(p[1]), p[2], p[3]))
test_fold = temp_pairs
...
teacher_classifier = PreClassifyAtt(hidden_size=hidden_size)
...
for epoch in range(n_epochs):
    ...
    input_batches, input_lengths, output_batches, output_lengths, nums_batches, \
        num_stack_batches, num_pos_batches, num_size_batches, num_value_batches = prepare_train_batch(train_pairs, batch_size)
    print("epoch:", epoch + 1)
    ...
    for idx in range(len(input_lengths)):
        loss = train_tree(
            ...
            encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer, output_lang, num_pos_batches[idx], teacher_encoder,teacher_classifier,epoch,if_teacher = True)
        ...
        loss_teacher = teacher_pretrain(
                    ...
                    num_pos_batches[idx],fake_batches[idx],teacher_encoder,teacher_classifier, teacher_optimizer, if_multi=True)
        loss_total += loss
