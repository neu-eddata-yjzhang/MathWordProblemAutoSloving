...
class PreClassify(nn.Module):
    def __init__(self, hidden_size):
        #self.gru_output = nn.GRU(hidden_size, hidden_size, 2, dropout=0.4, bidirectional=True)
        super(PreClassify, self).__init__()
        self.gru_input = nn.GRU(hidden_size, hidden_size, 2, dropout=0.4, bidirectional=True)
        self.hidden_size = hidden_size
        #self.down_sample = nn.Linear(hidden_size * 2, hidden_size)
...
class TeacherEncoderAttention(nn.Module):
    ...
    def forward(self, input_seqs, input_lengths, hidden=None):
        ...
        # print(attn_output.size()) #torch.Size([13, 50, 128])
        # print('outputs',outputs.size())
        # print('attn_output',attn_output.size())
        # S x B x H
        return attn_output 

class PreClassifyAtt(nn.Module):
    def __init__(self,hidden_size):
        super(PreClassifyAtt, self).__init__()
        hidden_size = 128
        self.gru_input = nn.GRU(512, hidden_size, 2, dropout=0.4, bidirectional=True)
        self.hidden_size = hidden_size
        self.fc1 = nn.Linear(hidden_size * 2, hidden_size * 2)
        self.fc1_1 = nn.Linear(hidden_size * 2, hidden_size)
        self.relu = nn.LeakyReLU(0.2)
    ...
    def forward(self, encoder_outputs, teacher_encoder_outputs):
        ...
        encoder_outputs = torch.mean(student_feature.transpose(1,0),dim = 1)
        teacher_encoder_outputs = torch.mean(teacher_encoder_outputs.transpose(1,0),dim = 1)
        # print(encoder_outputs.size(),teacher_encoder_outputs.size())
        # exit()
        merged_outputs = torch.cat([encoder_outputs,teacher_encoder_outputs],dim = -1)
        #merged_outputs = self.relu(merged_outputs)
        merged_outputs = F.dropout(merged_outputs, p = 0.2)
...
class EncoderSeq(nn.Module):
    ...
    def forward(self, input_seqs, input_lengths,hidden=None):
        ...
        # output_list.append(pade_outputs)
        #print(pade_outputs.size(),problem_output.size())
...
class Prediction(nn.Module):
    ...
    def forward(self, node_stacks, left_childs, encoder_outputs, num_pades, padding_hidden, seq_mask, mask_nums):
        ...
        #print(self.embedding_weight.size()) torch.Size([1, 2, 512])
        embedding_weight = self.embedding_weight.repeat(*repeat_dims)  # B x input_size x N
        #print(embedding_weight.size()) torch.Size([50, 2, 512])
        #print(num_pades.size()) #torch.Size([50, 12, 512])
