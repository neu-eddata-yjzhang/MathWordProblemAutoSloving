...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               ...
               merge_optimizer, output_lang, num_pos,teacher_encoder,teacher_classifier,epoch,if_teacher = True, english=False):
    ...
    encoder_outputs_list, problem_output = encoder(input_var, input_length)
    encoder_outputs = encoder_outputs_list[-1]
    # Prepare input and output variables
    node_stacks = [[TreeNode(_)] for _ in problem_output.split(1, dim=0)]
    teacher_target = copy.deepcopy(target)
    true_encoder_outputs = teacher_encoder(teacher_target.cuda(),target_length)
    max_target_length = max(target_length)
    #teacher_loss = 0
    all_node_outputs = []
    # all_leafs = []
    ...
    all_nums_encoder_outputs = get_all_number_encoder_outputs(encoder_outputs, num_pos, batch_size, num_size,
                                                              encoder.hidden_size)
    #print(all_nums_encoder_outputs.size()) torch.Size([50, 5, 512])
    ...
    for t in range(max_target_length):
        ...
        for idx, l, r, node_stack, i, o in zip(range(batch_size), left_child.split(1), right_child.split(1),
            ...
            if i < num_start:
                ...
                # print(current_nums_embeddings.size())  #torch.Size([50, 7, 512])
...
def evaluate_tree(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos,beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
...
def teacher_pretrain(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, output_lang, num_pos, fake_batch, teacher_encoder, teacher_classifier, teacher_optimizer, if_multi = False, english=False):
    # sequence mask for attention
    max_len = max(input_length)
    ...
    # Run words through encoder
    encoder_outputs_list, _ = encoder(input_var, input_length)
    fake_encoder_outputs = teacher_encoder(fake_var,target_length)
    true_encoder_outputs = teacher_encoder(target,target_length)
    loss = 0
