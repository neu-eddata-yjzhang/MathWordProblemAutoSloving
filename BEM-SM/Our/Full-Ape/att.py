...
valid_data ,valid_ids = load_data('data/ape/valid.ape.json',1)
train_data ,train_ids= load_data('data/ape/train.ape.json',1)
test_data ,test_ids= load_data('data/ape/test.ape.json',1)
data = valid_data + train_data + test_data

def get_train_test_fold(data,pairs,id_list,tes_id_list):
    train_id = id_list
    test_id = tes_id_list
    train_fold = []
    test_fold = []
...
train_fold, test_fold= get_train_test_fold(data,pairs,train_ids,test_ids)
...
for epoch in range(n_epochs):
    ...
    with torch.no_grad():
        if epoch % 1 == 0 or epoch > n_epochs - 80:
            ...
            if epoch == n_epochs - 1:
                with open('best_result.json','w',encoding='utf-8') as zyjjjj:
