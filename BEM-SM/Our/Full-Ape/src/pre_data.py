...
def prepare_data(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False,chinese=True):
    ...
    train_pairs = []
    test_pairs = []
    tokenizer = BertTokenizer.from_pretrained("/home/yjzhang/Paper/Model/full-bert-60")
    ...
    for pair in pairs_trained:
        ...
        train_pairs.append((input_cell, inputs['input_ids'].squeeze().size(0), output_cell, len(output_cell),
                            pair[2], num_pos, num_stack, inputs)) #pair[4] groupnum
...
def ape_get_train_test_fold(ori_path,prefix,data,pairs,ape_path,ape_id,ape_test_id):
    i = 0
...
def load_data(filename,divide=1):
    ...
    for l in open(filename):
        ...
        if question_num%divide==0:
            l = json.loads(l)
            question, equation, answer,id = l['segmented_text'].strip(), l['equation'], l['ans'],l["id"]
            ids.append(id)
            flag=0
            ...
            try:
                ...
                #print(question)
                #print(equation)
                D.append((question, equation, answer,id))
                cannot_eval+=1
                continue
