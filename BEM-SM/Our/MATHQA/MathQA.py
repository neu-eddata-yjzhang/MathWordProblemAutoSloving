...
train_data = json.load(open('data/MathQA_bert_token_train.json', 'r', encoding='utf-8'))
val_data1 = json.load(open('data/MathQA_bert_token_test.json', 'r', encoding='utf-8'))
test_data1 = json.load(open('data/MathQA_bert_token_val.json', 'r', encoding='utf-8'))
pairs_trained, generate_nums, copy_nums = make_pair(train_data, False)
pairs_tested, _, _ = make_pair(test_data1, True)
pairs_valed, _, _ = make_pair(val_data1, True)
    ...
    for idx in range(len(input_lengths)):
        ...
        for pos in range(len(num_size_batches[idx])):
            ...
            if output_batches[idx][pos][0] >= 7:
                continue
            #print(num_size_batches[idx][pos])
        ...
        if epoch % 1 == 0 or epoch > n_epochs - 80:
            ...
            for test_batch in test_pairs:
                ...
                test_res = evaluate_tree(test_batch[0], test_batch[1], generate_num_ids, encoder, predict, generate,
                                        merge, output_lang, test_batch[5],beam_size=beam_size,bert_input=test_batch[7])
                val_ac, equ_ac, zyj, ch = compute_prefix_tree_result(test_res, test_batch[2], output_lang, test_batch[4], test_batch[6])
                if val_ac:
                    value_ac += 1
                if equ_ac:
                    equation_ac += 1
            ...
            if epoch == n_epochs - 1:
                with open('best_result2.json','w',encoding='utf-8') as zyjjjj:
                    json.dump(count_list,zyjjjj)
