...
def generate_tree_input(target, decoder_output, nums_stack_batch, num_start, unk):
    ...
    for i in range(len(target)):
        if target[i] == unk:
            num_stack = nums_stack_batch[i][-1]
...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               ...
               merge_optimizer, output_lang, num_pos, teacher_encoder,teacher_classifier,epoch, bert_input,if_teacher = True, english=False):
    ...
    encoder_outputs_list, problem_output = encoder(input_var, input_length, bert_input)
    encoder_outputs = encoder_outputs_list[-1]
    #print(encoder_outputs.size(),problem_output.size()) # Source:torch.Size([84, 50, 768])
    #bert:torch.Size([89, 50, 768]) torch.Size([50, 768])
    # Prepare input and output variables
    node_stacks = [[TreeNode(_)] for _ in problem_output.split(1, dim=0)]
    teacher_target = copy.deepcopy(target)
    true_encoder_outputs,_ = teacher_encoder(teacher_target.cuda(),target_length)
    max_target_length = max(target_length)
    #teacher_loss = 0
    all_node_outputs = []
    # all_leafs = []
    #prediction_true = teacher_classifier(encoder_outputs,true_encoder_outputs)
    #teacher_loss += f.cross_entropy(prediction_true,torch.ones(prediction_true.size(0)).long().cuda())
    prediction_true = teacher_classifier(encoder_outputs,true_encoder_outputs)
    teacher_loss = f.cross_entropy(prediction_true,torch.ones(prediction_true.size(0)).long().cuda()) #/ len(encoder_outputs_list)
    copy_num_len = [len(_) for _ in num_pos]
    num_size = max(copy_num_len)
    all_nums_encoder_outputs = get_all_number_encoder_outputs(encoder_outputs, num_pos, batch_size, num_size,
                                                              encoder.hidden_size)
    #print(all_nums_encoder_outputs.size()) torch.Size([50, 7, 768])
    ...
    for t in range(max_target_length):
        ...
        for idx, l, r, node_stack, i, o in zip(range(batch_size), left_child.split(1), right_child.split(1),
            ...
            if i < num_start:
                ...
                #print(current_nums_embeddings.size()) torch.Size([50, 2, 768])
    ...
    #torch.nn.utils.clip_grad_norm_(merge.parameters(), 5)
...
def evaluate_tree(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos, bert_input,beam_size=5,english=False, max_length=MAX_OUTPUT_LENGTH):
    ...
    encoder_outputs_list, problem_output = encoder.evaluate(input_var, [input_length], bert_input)
    encoder_outputs = encoder_outputs_list[-1]
    # Prepare input and output variables
    node_stacks = [[TreeNode(_)] for _ in problem_output.split(1, dim=0)]
...
def teacher_pretrain(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, output_lang, num_pos, fake_batch,teacher_encoder, teacher_classifier, teacher_optimizer, bert_input,if_multi = False, english=False):
    # sequence mask for attention
    max_len = max(input_length)
    ...
    fake_var = torch.LongTensor(fake_batch).transpose(0, 1)
    ...
    if USE_CUDA:
        #input_var = input_var.cuda()
        fake_var = fake_var.cuda()
        target = target.cuda()

    # Zero gradients of both optimizers
    teacher_optimizer.zero_grad()
    # Run words through encoder
    encoder_outputs_list, _ = encoder(input_var, input_length,bert_input)
    fake_encoder_outputs, _ = teacher_encoder(fake_var,target_length)
    true_encoder_outputs, _ = teacher_encoder(target,target_length)
    loss = 0

    prediction_true = teacher_classifier(encoder_outputs_list[-1].data,true_encoder_outputs)
    prediction_fake = teacher_classifier(encoder_outputs_list[-1].data,fake_encoder_outputs)

    loss = 0.6 * f.cross_entropy(prediction_true,torch.ones(prediction_true.size(0)).long().cuda()) + 0.4 * f.cross_entropy(prediction_fake,torch.zeros(prediction_fake.size(0)).long().cuda())
    loss.backward()
...
def evaluate_tree_(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos, batch_graph, beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
    ...
    encoder_outputs_list, problem_output = encoder(input_var, [input_length], batch_graph)
    encoder_outputs = encoder_outputs_list[-1]
    # Prepare input and output variables
    node_stacks = [[TreeNode(_)] for _ in problem_output.split(1, dim=0)]
    ...
    for t in range(max_length):
        ...
        if flag:
            break
    return beams[0].out, problem_output.view(-1).cpu().detach().numpy()
    #return beams[0].out, encoder_outputs.squeeze().mean(0).view(-1).cpu().detach().numpy()