...
def prepare_data(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False,chinese=True):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_pairs = []
    tokenizer = BertTokenizer.from_pretrained("/home/yjzhang/BEM-SM/Model/MacBert_23K_cl")
    ...
    for pair in pairs_trained:
        ...
        train_pairs.append((input_cell, inputs['input_ids'].squeeze().size(0), output_cell, len(output_cell),
                            pair[2], num_pos, num_stack, pair[4],inputs)) #pair[4] groupnum
    ...
    for pair in pairs_tested:
        ...
        test_pairs.append((input_cell, inputs['input_ids'].squeeze().size(0), output_cell, len(output_cell),
                           pair[2], num_pos, num_stack,pair[4],inputs))
...
def prepare_train_batch(pairs_to_batch, batch_size):
    ...
    for batch in batches:
        ...
        for _, i, _, j, _, _, _,_,_ in batch:
            input_length.append(i)           #train_pairs.append((input_cell, len(input_cell), output_cell, len(output_cell),pair[2], pair[3], num_stack, pair[4]))
        ...
        output_batch = []
        num_batch = []
        num_stack_batch = []  # Important
        num_pos_batch = []
        num_size_batch = []
        group_batch = []
        num_value_batch = []
        bert_input_batch = []
        for i, li, j, lj, num, num_pos, num_stack, group, bert_input in batch:
    ...
    return input_batches, input_lengths, output_batches, output_lengths, nums_batches, num_stack_batches, num_pos_batches, num_size_batches, num_value_batches, bert_input_batches
...
def ape_get_train_test_fold(ori_path,prefix,data,pairs,group,ape_path,ape_id,ape_test_id):
    i = 0
