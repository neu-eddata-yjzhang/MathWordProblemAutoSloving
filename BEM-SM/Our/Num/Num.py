...
# pairs_tested = test_fold[:100]
# pairs_trained = train_fold[100:200]
...
for epoch in range(n_epochs):
    ...
    loss_total = 0
    input_batches, input_lengths, output_batches, output_lengths, nums_batches, num_stack_batches, num_pos_batches, num_size_batches, bert_batches,nn_batches = prepare_train_batch(train_pairs, batch_size)
    print("epoch:", epoch + 1)
    start = time.time()
    for idx in range(len(input_lengths)):
        loss = train_tree(
            ...
            encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer, output_lang, num_pos_batches[idx], bert_batches[idx],nn_batches[idx])
        loss_total += loss
    ...
    print("--------------------------------")
    if epoch % 1 == 0 or epoch > n_epochs - 10:
        value_ac = 0
        equation_ac = 0
        eval_total = 0
        start = time.time()
        for test_batch in test_pairs:
            ...
            test_res = evaluate_tree(test_batch[0], test_batch[1], generate_num_ids, encoder, predict, generate,
                                         merge, output_lang, test_batch[5], test_batch[7],test_batch[4], beam_size=beam_size)
