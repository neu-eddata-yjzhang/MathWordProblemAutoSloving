...
def transfer_num_boost_57k(data):  # transfer num into "NUM"
    ...
    for d in data:
        ...
        pairs.append((input_seq, answer, nums, num_pos, id2))
...
def transfer_num_test(data):  # transfer num into "NUM"
    ...
    for d in data:
        ...
        if is_contains_chinese(str(input_seq)) == False:
            t3 += 1
            pairs.append(0)
            continue
...
def prepare_data_23k(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    ...
    test_pairs = []
    test_pairs_ape = []
    tokenizer = BertTokenizer.from_pretrained("/home/yjzhang/Paper/Model/RoBerta")
...
def prepare_data(pairs_trained, pairs_tested, pairs_tested_ape, trim_min_count, generate_nums, copy_nums, tree=False):
    ...
    for pair in pairs_tested_ape:
        ...
        if len(input_cell) > 100 or len(output_cell) > 20:
            continue
        test_pairs_ape.append((input_cell, inputs['input_ids'].squeeze().size(0), output_cell, len(output_cell),
                           pair[2], num_pos, num_stack, inputs))
    print('Number of testing data %d' % (len(test_pairs_ape)))
    
    return input_lang, output_lang, train_pairs, test_pairs, test_pairs_ape


def prepare_data_rbt(pairs_trained, pairs_tested, pairs_tested_ape, trim_min_count, generate_nums, copy_nums, tree=False):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_pairs = []
...
def prepare_data_with57k(pairs_trained, pairs_tested, pairs_tested_ape, pairs_57k, trim_min_count, generate_nums, copy_nums, tree=False):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_pairs = []
    ...
    for pair in pairs_57k:
        ...
        ans = pair[1]
        id_2 = pair[4]
        test_pairs_57k.append((input_cell, inputs['input_ids'].squeeze().size(0), ans, id_2,
                           pair[2], num_pos, num_stack, inputs))
    print('Number of 57k testing data %d' % (len(test_pairs_57k)))

    return input_lang, output_lang, train_pairs, test_pairs_57k
...
def prepare_train_batch(pairs_to_batch, batch_size):
    ...
    for batch in batches:
        ...
        nn_batches.append(nn_batch)
    return input_batches, input_lengths, output_batches, output_lengths, nums_batches, num_stack_batches, num_pos_batches, num_size_batches, bert_input_batches,nn_batches
...
def prepare_data_old(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_pairs = []
...
def prepare_train_batch_old(pairs_to_batch, batch_size):
    pairs = copy.deepcopy(pairs_to_batch)
    random.shuffle(pairs)  # shuffle the pairs
    pos = 0
    input_lengths = []
