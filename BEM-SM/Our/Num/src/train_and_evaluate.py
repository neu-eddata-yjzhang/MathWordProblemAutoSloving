...
def raw_data(filename):  # load the json data to list(dict()) for MATH 23K
    print("Reading lines...")
...
def raw_data_57k(filename):  # load the json data to list(dict()) for MATH 23K
    print("Reading lines...")
    f = open(filename, encoding="utf-8")
    js = ""
    data = []
...
def raw_data_new(filename, id_list, overlap_list):  # load the json data to list(dict()) for MATH 23K
    print("Reading lines...")
    f = open(filename, encoding="utf-8")
    js = ""
    data = []
    for i, s in enumerate(f):
        ...
        if "千米/小时" in data_d["equation"]:
            data_d["equation"] = data_d["equation"][:-5]
        data_d["type"] = "ape"
        if data_d['id'] in id_list and data_d['id'] not in overlap_list:
            data.append(data_d)
        js = ""

    return data

def raw_data_pretrain_train(filename, overlap_list):  # load the json data to list(dict()) for MATH 23K
    print("Reading lines...")
    f = open(filename, encoding="utf-8")
    js = ""
    data = []
    for i, s in enumerate(f):
        ...
        if "千米/小时" in data_d["equation"]:
            data_d["equation"] = data_d["equation"][:-5]
        data_d["type"] = "ape_train"
        if data_d['id'] not in overlap_list:
            data.append(data_d)
        js = ""

    return data

def raw_data_pretrain_test(filename, overlap_list):  # load the json data to list(dict()) for MATH 23K
    print("Reading lines...")
    f = open(filename, encoding="utf-8")
    js = ""
    data = []
...
def raw_data_all(filename):  # load the json data to list(dict()) for MATH 23K
    print("Reading lines...")
    f = open(filename, encoding="utf-8")
    js = ""
    data = []
...
def compute_result_weak(test_res, tar_value, output_lang, num_list):
    test = out_expression_list(test_res, output_lang, num_list)
    if test is None:
        return False
    try:
        #print(abs(compute_prefix_expression(test) - tar_value))
        if abs(float(compute_prefix_expression(test)) - float(tar_value)) < 1e-3:
            return True
    except:
        return False
...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               ...
               merge_optimizer, output_lang, num_pos, bert_input,nums, english=False):
    ...
    encoder_outputs, problem_output = encoder(input_var, input_length, bert_input,num_pos,nums)
...
def evaluate_tree(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos,
                  bert_input, nums,beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
    ...
    encoder_outputs, problem_output = encoder.evaluate(input_var, [input_length], bert_input,num_pos,nums)
...
def prepare_train_batch_cur(pairs_to_batch, batch_size):
    pairs = copy.deepcopy(pairs_to_batch)
    pairs = sorted(pairs, key=lambda tp: tp[3])
...
def prepare_train_batch_cur_2(pairs_to_batch, batch_size, epoch):
    pairs = copy.deepcopy(pairs_to_batch)
    pairs = sorted(pairs, key=lambda tp: (tp[3], tp[1]))
    if epoch < 10:
        pairs = pairs[:int(0.2 * len(pairs))]
    elif epoch >= 10 and epoch < 20:
        pairs = pairs[:int(0.4 * len(pairs))]
    elif epoch >= 20 and epoch < 30:
        pairs = pairs[:int(0.6 * len(pairs))]
...
def get_train_test_fold(ori_path,prefix,data,pairs,group,ape_path,ape_id,ape_test_id):
    ...
    for item,pair in zip(data, pairs):
        pair = list(pair)
        pair = tuple(pair)
        if item['id'] in train_id and item['type'] == '23k':
            i += 1
            train_fold.append(pair)
        elif item['id'] in test_id and item['type'] == '23k':
            test_fold.append(pair)
        elif str(item['id']) in id_list and item['type'] == 'ape':
            train_fold.append(pair)
        elif str(item['id']) in test_id_list and item['type'] == 'ape':
