...
from .group_att import GroupAttention, PositionwiseFeedForwardd
...
class Bert_EncoderSeq(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, n_layers=2, dropout=0.5):
        ...
        self.bert_rnn = BertModel.from_pretrained("/home/yjzhang/Paper/Model/MacBert")
        #self.bert_rnn = BertModel.from_pretrained("Your Path")

        self.group_attention = GroupAttention(8, 768)
        ff = PositionwiseFeedForwardd(768,768,dropout)
        self.onelayer = Encoder(EncoderLayer(768,deepcopy(self.group_attention),deepcopy(ff),dropout),1)
    ...
    def forward(self, input_seqs, input_lengths, bert_encoding, hidden=None):
        ...
        #assert pade_outputs.size()==(bsz,max_seq_length,512)#sxh!!!!!!!!!!!!
        src_mask = self.group_attention.get_mask(src=input_ids)#sxh!!!!!!!
        bert_output = self.onelayer(bert_output.transpose(0,1), src_mask.cuda())#sxh!!!!!!!
        # print(bert_output.transpose.size())
        # print('xxxx')
        # exit()
        output_list.append(bert_output.transpose(0,1))

        #print(bert_output.size(),problem_output.size())
        return output_list, problem_output
    def evaluate(self, input_seqs, input_lengths, bert_encoding):
        ...
        return output_list, problem_output #seq_len, batch_size, H(768)


class TeacherEncoderRNN(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, n_layers=2, dropout=0.2):
        super(TeacherEncoderRNN, self).__init__()
...
class Prediction(nn.Module):
    ...
    def forward(self, node_stacks, left_childs, encoder_outputs, num_pades, padding_hidden, seq_mask, mask_nums):
        ...
        #print(self.embedding_weight.size()) torch.Size([1, 2, 768])
        embedding_weight = self.embedding_weight.repeat(*repeat_dims)  # B x input_size x N
        #print(embedding_weight.size()) torch.Size([50, 2, 768])
        #print(num_pades.size()) torch.Size([50, 0, 768])
...
class EncoderSeq(nn.Module):
    ...
    def forward(self, input_seqs, input_lengths, batch_graph, hidden=None):
        ...
        embedded = self.em_dropout(embedded)
        #if self.train():
        #    embedded = embedded + 0.2 * torch.randn(embedded.size()).cuda()
        ...
        output_list.append(pade_outputs)
