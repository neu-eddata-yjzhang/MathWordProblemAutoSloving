...
for epoch in range(n_epochs):
    input_batches, pos_input_batches, neg_input_batches, OpNum_batches = prepare_train_batch_all(train_pairs, batch_size,epoch)
    print("epoch:", epoch + 1)
    start = time.time()
    #result_list = []
    cl_total_loss = 0
    ...
    for idx in range(len(input_batches)):
        ...
        src, op_loss_src = pre_bert(input_batches[idx], OpNum_batches[idx], device = device)
        pos, op_loss_pos = pre_bert(pos_input_batches[idx], OpNum_batches[idx], device = device)
        neg, op_loss_neg = pre_bert(neg_input_batches[idx], OpNum_batches[idx], device = device)
        ...
        op_loss = op_loss_src + op_loss_pos + op_loss_neg
        cl_loss = contra_loss_func(pos_sim, neg_sim, target)
        loss = cl_loss + op_loss
        cl_total_loss += cl_loss.item()
        op_total_loss += op_loss.item()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(pre_bert.parameters(), 5)
        model_optimizer.step()
    print('train-CL loss:', cl_total_loss/len(input_batches))
    print('train-Op loss:', op_total_loss/len(input_batches))
    ...
    op_total_loss = 0
    if epoch % 1 == 0:
        input_batches, pos_input_batches, neg_input_batches, OpNum_batches = prepare_train_batch_all(test_pairs, batch_size, epoch)
        with torch.no_grad():
            for idx in range(len(input_batches)):
                pre_bert.eval()
                src, op_loss_src = pre_bert(input_batches[idx], OpNum_batches[idx], device = device)
                ...
                cl_total_loss += cl_loss.item()
                op_total_loss += op_loss.item()
        print('test-CL loss:', cl_total_loss/len(input_batches))
        print('test-Op loss:', op_total_loss/len(input_batches))
        print("--------------------------------")
