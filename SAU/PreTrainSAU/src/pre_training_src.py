...
def prepare_data_pretraining_all(pairs_trained, pairs_tested, pairs_tested_ape, trim_min_count, generate_nums, copy_nums, tree=False):
    ...
    test_pairs = []
    test_pairs_ape = []
    tokenizer = BertTokenizer.from_pretrained("/home/yjzhang/BEM-SM/Model/MacBert")
    ...
    for pair in pairs_trained:
        ...
        try:
            ...
            train_pairs.append((input_cell, inputs['input_ids'].squeeze().size(0), output_cell, len(output_cell),
                                pair[2], num_pos, num_stack, inputs, op_nums,len(pair[4])))
        except:
            continue
...
def prepare_train_batch_all(pairs_to_batch, batch_size):
    ...
    for batch in batches:
        ...
        for i, li, j, lj, num, num_pos, num_stack, bert_input, op_nums,numtype in batch:
            ...
            if numtype == 1:
                num_type_batch.append([0.0,1.0])
            else:
                num_type_batch.append([1.0,0.0])
...
class PreTrainBert_all_Mac(nn.Module):
    def __init__(self, hidden_size, batch_size):
        ...
        self.bert_model = BertModel.from_pretrained("/home/yjzhang/BEM-SM/Model/MacBert")
    ...
    def forward(self, bert_input, num_pos,op_num,numtype,device,check=False):
        ...
        loss_1 = self.loss_function(self.fc_num_num(problem_output), torch.tensor(numtype).cuda().to(device))
        loss_2 = self.dist_loss(self.fc_op_num(problem_output), torch.FloatTensor(op_num).to(device))#.cuda())

        # print(self.softmax(self.fc_num_num(problem_output).unsqueeze(0)).shape) #[1,64,2]
        # print(torch.tensor([numtype]).cuda().to(device).shape)#[1,64,2]

        # pre = self.softmax(self.fc_num_num(problem_output).unsqueeze(0))[0]
        # lable = torch.tensor([numtype]).cuda().to(device)[0]
        # print(pre.shape,lable.shape)
        # for i in range(pre.shape[0]):
        #     print(pre[i],lable[i],self.loss_function(pre[i].unsqueeze(0),lable[i].unsqueeze(0)))
