...
def teacher_pretrain(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
    ...
    prediction_true = teacher_classifier(encoder_outputs_list.data,true_encoder_outputs)
    prediction_fake = teacher_classifier(encoder_outputs_list.data,fake_encoder_outputs)
...
def train_tree_with_subtree_semantic_alignment(epoch,input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch,bert_input,fake_batches,teacher_encoder,teacher_classifier, teacher_optimizer,generate_nums,
    ...
    # Turn padded arrays into (batch_size x max_len) tensors, transpose into (max_len x batch_size)
    #input_var = torch.LongTensor(input_batch).transpose(0, 1) #src
    input_var = None
    target = torch.LongTensor(target_batch).transpose(0, 1)
    ...
    encoder_outputs, problem_output = encoder(input_var, input_length,bert_input)
    #print(encoder_outputs.shape,problem_output.shape) # src: torch.Size([66, 32, 512]) torch.Size([32, 512])
    ###Bert torch.Size([68, 32, 768]) torch.Size([32, 768])
...
def evaluate_tree(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos,bert_input,
    ...
    # Run words through encoder
    encoder_outputs, problem_output = encoder.evaluate(input_var, [input_length],bert_input)
